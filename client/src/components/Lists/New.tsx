...
export default function NewProducts() {
  ...
  return (
    <div className="mt-8 flex h-max flex-col gap-8 text-center">
      ...
      <div className="mt-4 grid grid-cols-1 grid-rows-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {cardData.map((card) => (
          <Card productName={card.productName} price={card.price} />
        ))}
      </div>
      <button className="group relative inline-block w-[200px] self-center overflow-hidden bg-brand px-5 py-5 font-medium text-black">
